...
int main(int argc, char** argv) {
  ...
  nh_private.param("left_camera_namespace", left_camera_name, std::string("left"));
  nh_private.param("right_camera_namespace", right_camera_name, std::string("right"));
  nh_private.param("output_frame", output_frame, std::string("rect/") + left_camera_name);
  int frame_skip;
  ...
  stereo_info_remap["raw/left/image"]= ros::names::resolve(std::string("raw/") + left_camera_name + "/image");
  stereo_info_remap["raw/right/image"]= ros::names::resolve(std::string("raw/") + right_camera_name + "/image");

  stereo_info_remap["raw/left/camera_info"]= ros::names::resolve(std::string("raw/") + left_camera_name + "/camera_info");
  stereo_info_remap["raw/right/camera_info"]= ros::names::resolve(std::string("raw/") + right_camera_name + "/camera_info");

  stereo_info_remap["rect/left/camera_info"]= ros::names::resolve(std::string("rect/") + left_camera_name + "/camera_info");
  stereo_info_remap["rect/right/camera_info"]= ros::names::resolve(std::string("rect/") + right_camera_name + "/camera_info");

  std::string stereo_info_name = ros::this_node::getName(); //leave in node namespace to get input parameters
  manager.load(stereo_info_name, "image_undistort/StereoInfoNodelet", stereo_info_remap, nargv);
  ROS_INFO_STREAM("Started " << stereo_info_name << "_stereo_info" << " nodelet.");
  ...
  manager.load(left_name, "image_undistort/ImageUndistortNodelet", left_remap, nargv);
  ...
  manager.load(right_name, "image_undistort/ImageUndistortNodelet", right_remap, nargv);
  ...
  disparity_remap["left/image_rect"] = std::string("rect/") + left_camera_name + "/image";
  disparity_remap["right/image_rect"] = std::string("rect/") + right_camera_name + "/image";
  disparity_remap["left/camera_info"] = std::string("rect/") + left_camera_name + "/camera_info";
  disparity_remap["right/camera_info"] = std::string("rect/") + right_camera_name + "/camera_info";

  std::string disparity_name = ros::this_node::getName() + "_disparity";
  ros::param::set(disparity_name, disparity_params);
  manager.load(disparity_name, "stereo_image_proc/disparity", disparity_remap, nargv);
  ...
  manager.load(pointcloud_name, "stereo_image_proc/point_cloud2", pointcloud_remap, nargv);
